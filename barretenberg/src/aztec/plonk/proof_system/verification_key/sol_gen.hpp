...
inline void output_vk_sol(std::ostream& os, std::shared_ptr<verification_key> const& key, std::string const& class_name)
{
    ...
    print_g1("0x100", key->constraint_selectors.at("Q_M"), "//vk.QM");
    print_g1("0x120", key->constraint_selectors.at("Q_C"), "//vk.QC");
    print_g1("0x140", key->permutation_selectors.at("SIGMA_1"), "//vk.SIGMA1");
    print_g1("0x160", key->permutation_selectors.at("SIGMA_2"), "//vk.SIGMA2");
    print_g1("0x180", key->permutation_selectors.at("SIGMA_3"), "//vk.SIGMA3");
    os <<
      "            mstore(add(vk, 0x1a0), " << (key->contains_recursive_proof ? "0x01" : "0x00") << ") // vk.contains_recursive_proof\n"
      "            mstore(add(vk, 0x1c0), " << (key->contains_recursive_proof ? key->recursive_proof_public_input_indices[0] : 0) << ") // vk.recursive_proof_public_input_indices\n"
      "            mstore(mload(add(vk, 0x1e0)), " << key->reference_string->get_g2x().x.c1 << ") // vk.g2_x.X.c1\n"
      "            mstore(add(mload(add(vk, 0x1e0)), 0x20), " << key->reference_string->get_g2x().x.c0 << ") // vk.g2_x.X.c0\n"
      "            mstore(add(mload(add(vk, 0x1e0)), 0x40), " << key->reference_string->get_g2x().y.c1 << ") // vk.g2_x.Y.c1\n"
      "            mstore(add(mload(add(vk, 0x1e0)), 0x60), " << key->reference_string->get_g2x().y.c0 << ") // vk.g2_x.Y.c0\n"
      "        }\n"
      "        return vk;\n"
      "    }\n"
      "}\n";
...
/**
 * Write a solidity file containing the method that returns the verification key to the given stream.
 **/
inline void output_vk_sol_method(std::ostream& os, std::shared_ptr<verification_key> const& key)
{
    const auto print_u256 =
        [&](const std::string& name, const barretenberg::fr& element, const std::string& postlabel) {
            os << "            " << name << element << postlabel << std::endl;
    ...
    // XXX: For some reason, the first part gets mangled when we try to read it from Rust
    // Temp fix is to duplicate the first line and then truncate it on the Rust side
